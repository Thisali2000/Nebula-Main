...
$(function(){
        ...
        $.ajax({
            ...
            success: function(res){
                if(res && res.success){
                    ...
                    res.data.forEach(function(intk){ $intake.append(`<option value="${intk.intake_id}">${intk.intake_name}</option>`); });
    ...
    // Show the table section once a student is selected
    $('#ind_studentDropdown').on('change', function(){
        if($(this).val()){
            $('#individualClearanceTableSection').show();
        } else {
            $('#individualClearanceTableSection').hide();
        }
    });
    ...
    $(document).on('click', '.send-individual-clearance-btn', function(){
        ...
        if(!payload.location || !payload.course_id || !payload.intake_id || !payload.student_id){
            return alert('Please select Location, Course, Intake and Student first.');
        }
        ...
        $.ajax({
            ...
            data: payload,
            success: function(res){
                const msg = (res && res.message) ? res.message : 'Request sent';
                // reuse existing toast
                const t = `<div class="toast align-items-center text-white bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true"><div class="d-flex"><div class="toast-body">${msg}</div><button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Close"></button></div></div>`;
                $('.toast-container').append(t);
                $('.toast').last().toast({ delay:3000, autohide:true }).toast('show');
            },
            error: function(){
                const t = `<div class="toast align-items-center text-white bg-danger border-0" role="alert" aria-live="assertive" aria-atomic="true"><div class="d-flex"><div class="toast-body">Failed to send the request</div><button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Close"></button></div></div>`;
                $('.toast-container').append(t);
                $('.toast').last().toast({ delay:3000, autohide:true }).toast('show');
            },
            complete: function(){
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\thisali\Desktop\thisali\Nebula-Main\resources\views/all_clearance.blade.php ENDPATH**/ ?>